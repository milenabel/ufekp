...
%   results/sphere/<subdir>/error_vs_N_sphere_phs_sweep.png
    ...
    phs_colors = lines(max(1, numel(PHScurves)));  % MATLAB's 'lines' colormap
    ...
    if any(isfinite(el2_fs))
        H(end+1) = semilogy(sN, el2_fs, '-', 'Color', cFS,  'Marker', mFS, ...
            'MarkerSize', ms, 'MarkerFaceColor', 'w', 'LineWidth', lw);  L{end+1}='FS';
    end
    if any(isfinite(el2_pls))
        H(end+1) = semilogy(sN, el2_pls,'-', 'Color', cPLS, 'Marker', mPLS, ...
            'MarkerSize', ms, 'MarkerFaceColor', 'w', 'LineWidth', lw);  L{end+1}='PLS';
    end
    ...
    for j = 1:numel(PHScurves)
        ...
        if any(isfinite(y) & y>0)
            ...
            H(end+1) = semilogy(sN, y, '-', 'Color', phs_colors(j,:), ...
                'Marker', '>', 'MarkerSize', ms, 'MarkerFaceColor','w', 'LineWidth', lw);
            L{end+1} = leg;
        end
    end
    ...
    title('Sphere: Relative l_2 error vs. N^{1/d}, C^2(R^3) Wendland Kernel', ...
          'Interpreter','tex','FontSize',18,'FontWeight','bold');
