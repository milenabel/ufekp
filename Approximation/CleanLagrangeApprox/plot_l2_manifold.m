...
function plotErrorVsN_manifold(res)
    ...
    % --- series (bold lines)
    if any(isfinite(yFS))
        H(end+1) = semilogy(x, yFS, '-', 'Color', cFS,  'Marker', mFS, ...
            'MarkerSize', ms, 'MarkerFaceColor', 'w', 'LineWidth', lw);  L{end+1}='FS';
    end
    if any(isfinite(yPLS))
        H(end+1) = semilogy(x, yPLS,'-', 'Color', cPLS, 'Marker', mPLS, ...
            'MarkerSize', ms, 'MarkerFaceColor', 'w', 'LineWidth', lw);  L{end+1}='PLS';
    end
    if any(isfinite(yPHS))
        H(end+1) = semilogy(x, yPHS,'-', 'Color', cPHS, 'Marker', mPHS, ...
            'MarkerSize', ms, 'MarkerFaceColor', 'w', 'LineWidth', lw);  L{end+1}='PHS+poly';
    end
    if ~isempty(yfit)
    ...
    title('Relative l_2 error vs. N^{1/d}, C^2(R^3) Wendland Kernel', ...
          'Interpreter','tex','FontSize',18,'FontWeight','bold');

    % legend (bold text + thicker box)
    if ~isempty(H)
