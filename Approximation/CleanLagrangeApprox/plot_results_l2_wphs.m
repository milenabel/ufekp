...
function plot_results_l2_wphs(function_name, subdir, base_results_dir, smoothness_idx, variety)
    ...
    % Load unified results (+ auto-merge PHS p=9 if present)
    ...
    if include_all_Kt
        ...
        if isfield(res,'el2_vs1'),  err_vals = [err_vals; res.el2_vs1(:,sm)];  end
        if isfield(res,'el2_vs2'),  err_vals = [err_vals; res.el2_vs2(:,sm)];  end
        if isfield(res,'el2_vs3'),  err_vals = [err_vals; res.el2_vs3(:,sm)];  end
    end
...
function plotErrorVsN(res, sm, xmin, xmax, err_min, err_max, include_all_Kt)
    ...
    el2_vs1_s = NaN(n,1); if isfield(res,'el2_vs1'), el2_vs1_s = res.el2_vs1(:,sm); end
    el2_vs2_s = NaN(n,1); if isfield(res,'el2_vs2'), el2_vs2_s = res.el2_vs2(:,sm); end
    el2_vs3_s = NaN(n,1); if isfield(res,'el2_vs3'), el2_vs3_s = res.el2_vs3(:,sm); end
    ...
    close(h);
        ...
        % 1D FC (VS) panel that shows the same PHS curve as FS 
    if dim == 1
        h = figure; set(h,'Color','none'); ax = gca;
        set(ax,'Color','none','FontSize',12,'LineWidth',1.2);
        H = gobjects(0); L = {};
        ...
        % FC (VS) family
        add_series(el2_vs1_s, 'VS1');
        if include_all_Kt
            add_series(el2_vs2_s, 'VS2');
            add_series(el2_vs3_s, 'VS3');
        end
...
function S = getPlotStyles(dim)
    ...
    S.VS1 = struct('Legend','K_t = 1e12','Color',[0.929 0.694 0.125],'Marker','^','MarkerSize',6,'MarkerFaceColor','w','LineStyle','-');
    S.VS2 = struct('Legend','K_t = 1e8','Color',[0.494 0.184 0.556],'Marker','x','MarkerSize',8,'MarkerFaceColor','none','LineStyle','-');
    S.VS3 = struct('Legend','K_t = 1e4','Color',[0.466 0.674 0.188],'Marker','+','MarkerSize',8,'MarkerFaceColor','none', 'LineStyle','-');
