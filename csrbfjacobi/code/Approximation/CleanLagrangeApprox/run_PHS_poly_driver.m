% run_PHS_poly_driver.m
% Compute global PHS + polynomial results on the same setups you use
...
elseif dim==2
    ...
    xe = [st_e.fullintnodes{7}; st_e.bdrynodes{7}]; % as in your scripts

    % Training nodes (match what you used for results)
...
% (Uncomment ONE block per dim that matches your previous experiment)
if dim==1
    syms x;       
...
for k = start_nodes:end_nodes
    ...
    % polynomial total degree (same rule as yours)
