...
function plot_results_l2(function_name, subdir, base_results_dir, smoothness_idx, variety)
    ...
    %
    % New: set make_fs_family_plot=true to also emit an FS-only panel (FS, FS-κeff, FS-eval).
    ...
    % Optional FS family panel (FS, FS-κeff, FS-eval only; optionally fs2/fs3 if you want)
    % if make_fs_family_plot
    %     plotErrorVsN_FSFamily(res, sm);
    % end
end
...
function plotErrorVsN(res, sm, xmin, xmax, err_min, err_max, include_all_Kt)
    ...
    marks = {'-o','-s','-^','-x','-+','-d','-v'}; % add a couple more markers
    H = gobjects(0); L = {};
    ...
    function add_if_valid(y, mk, lab)
        ...
        if any(good)
            ...
            H(end+1) = hl; L{end+1} = lab; %#ok<AGROW>
        end
    end
...
%% NEW: FS-only family panel (FS, FS-κeff, FS-eval)
function plotErrorVsN_FSFamily(res, sm)
    dim = res.dim;
    sNs = res.sNs(:);
    n = numel(sNs);

    % Pull just FS variants
    el2_fs1   = NaN(n,1); if isfield(res,'el2_fs1'),   el2_fs1   = res.el2_fs1(:,sm); end
    el2_fs_eff= NaN(n,1); if isfield(res,'el2_fs_eff'),el2_fs_eff= res.el2_fs_eff(:); end
    el2_fs_eval=NaN(n,1); if isfield(res,'el2_fs_eval'),el2_fs_eval= res.el2_fs_eval(:); end

    % y-lims from these three only
    vals = [el2_fs1(:); el2_fs_eff(:); el2_fs_eval(:)];
    ...
    h = figure; set(h,'Color','none'); ax = gca;
    set(ax,'Color','none','FontSize',12,'LineWidth',1.2);
    marks = {'-^','-d','-v'}; % distinct from main plot
    ...
    add_if_valid(el2_fs1,    marks{1}, 'FS (K_t=1e12)');
    add_if_valid(el2_fs_eff, marks{2}, 'FS-eff');
    add_if_valid(el2_fs_eval,marks{3}, 'FS-eval');
    ...
    % Title and axes
    title('FS family: Relative l_2 error vs. N^{1/d}','Interpreter','tex','FontWeight','bold','FontSize',16);
    xlim([min(sNs) max(sNs)]); ylim([ymin ymax]);

    outname = sprintf('error_vs_N_fs_family_s%d.png', sm);
    export_fig(gcf, fullfile(res.results_dir, outname), '-png','-r300','-transparent');
    close(h);
end
