...
if variety
    ...
    % one combined panel: PLS, Diag, FS(fs1) (+ PHS if available), FC(fc1)
...
function plotAssemblyOrEval_FSFC_with_PHS(res, sm, xmin, xmax, y_min, y_max, kind, which)
    ...
    if strcmpi(which,'fs') || strcmpi(which,'fc')
        labtag = iff(strcmpi(which,'fs'),'fs','fc');
        S = cell(1,3);
        for j=1:3
        ...
        if strcmpi(kind,'assembly')
            ...
            FC1 = isfield(res,'a_time_fc1') * res.a_time_fc1(:,sm); if isempty(FC1), FC1 = NaN(n,1); end
        else
            FS1 = isfield(res,'e_time_fs1') * res.e_time_fs1(:,sm); if isempty(FS1), FS1 = NaN(n,1); end
            FC1 = isfield(res,'e_time_fc1') * res.e_time_fc1(:,sm); if isempty(FC1), FC1 = NaN(n,1); end
        end
    end
...
switch lower(kind)
    case 'assembly'
        ...
        for tag = ["fs1","fs2","fs3","fc1","fc2","fc3"]
