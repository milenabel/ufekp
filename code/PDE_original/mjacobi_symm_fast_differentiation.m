function[d, varargout] = mjacobi_symm_fast_differentiation(c, a, alph, dorder, varargin)
...
for q = 1:length(dorder)
  ...
  if dorder(q) > 0
    ...
    for qq = 1:dorder(q)
      % I assume here that matlab's \ or mldivide takes advantage of sparse
      % triangular matrices for fast inversion. The documentation suggests that
      % it should.
      %D = [Cmat\D(2:N,:); zeros([1 size(D,2)])];
      rc = D(2:N,:);
      sc = qc*(uc\(lc\(pc*rc)));
