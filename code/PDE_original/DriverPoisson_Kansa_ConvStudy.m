...
for k=start_nodes:end_nodes    
    ...
    N = size(a,1);      
   if k>1
        ep = ep*1.2*(nthroot(length(xi)/length(st.fullintnodes{k-1}),dim));
        %ep = ep*2;
   end
    ...
    %% Get polynomial reproduction blocks
    Vt = [Lv(1:Ni,:).', Dv.'];    
    %Vt = [V(1:Ni,:).',Dv.'];
    ...
    if bctype==2
        ...
        zi = z(1:Ni,:);
        zt = z.';
        z0 = z(Ni+1:Ni+size(Dv,1),:);
        z1 = z(Ni+size(Dv,1)+1:end,:);
        zt1 = zt(:,1:size(Vt,2));
        zt2 = zt(:,size(Vt,2)+1:end);
    else
        z = [];
        zi = [];
        zt = [];
    ...
    if schur_method==1
        ...
        lhs_A = [Lap(1:Ni,:);D];
        lhs_B = full([[Lv(1:Ni,:),zi];[Dv,z0]]);        
        lhs_C = [Vt;zt1];
        lhs_D = [[sparse(N,N),z1];[zt2,sparse(size(z,2),size(z,2))]];
    ...
    sol = rbf(ep,rd)*c(1:Ni+Nb,:) + V*c(Ni+Nb+1:end,:);
    sol_inner = sol(1:Ni);
