function [el2,elinf,a_time,e_time,c_poly,cond_num,c_rbf, sparsity] = CSRBFGenManifold(x,y,ell,xe,alph,rbf,ep,tree,ye_true)
...
%% We're on a manifold. We have to use QR with column pivoting (or an SVD)
[Q,R,E] = qr(B,0);
tol = max(size(B)) * eps(norm(R,inf));
r   = find(abs(diag(R)) > tol, 1, 'last');
g = dA\y;
gq = Q(:,1:r)'*g;
c_poly1 = linsolve(R(1:r,1:r),gq,opts2);
c_poly= zeros(size(B,2),1);
