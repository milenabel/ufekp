% DM = DistanceMatrixCSRBFwt(dsites,ctrs,ep,tree)
...
%              site and the j-th center in the i,j position
  ...
  function DM = DistanceMatrixCSRBFwt(dsites,ctrs,ep,Tree)
  ...
  if M > N  % faster if more centers than data sites     
     ...
     [idxb,distb] = rangesearch(Tree,dsites,support);
     for i = 1:N                
        idx = idxb{i}; dist = distb{i};        
     ...
     [idxb,distb] = rangesearch(Tree,ctrs,support);
     for j = 1:M        
        idx = idxb{j}; dist = distb{j};
        newentries = length(idx);
        iend = iend + newentries;
